...
## 회고
Django를 사용하여 블로그 CRUD를 구현하면서 웹 애플리케이션 개발에 대한 실제 경험을 쌓을 수 있었습니다. 사용자 인증, 데이터 모델링, URL 매핑, 뷰 작성 등 다양한 측면에서의 개발 작업을 경험하며 Django의 강력한 기능과 생산성을 체감할 수 있었습니다. 또한 Django는 문서화가 잘 되어 있고 활발한 커뮤니티를 가지고 있어서 이를 통해 필요한 지식과 팁을 얻을 수 있었습니다. 아쉬웠던 점은 블로그 프로젝트를 진행하면서 개발 경험을 쌓을 수 있었지만, 아직은 완전히 실전과 유사한 상황에서의 경험은 부족했습니다. 더 복잡한 기능을 구현하거나 실제 사용자 요구사항에 맞춰 개발하는 경험을 통해 더 나은 개발자로 성장하고 싶습니다.

## 7월 5일
### 작업내용:
1. `app/settings.py` 에서 `LOGIN_URL`을 '/user/login'으로 설정했습니다. 이는 로그인이 필요한 뷰에 접근할 때 사용자를 로그인 페이지로 리디렉션하기 위한 URL입니다.
2. `form_error.html`: 폼 오류를 출력하기 위한 템플릿을 작성했습니다. 해당 템플릿은 폼 오류 메시지를 출력하는 역할을 합니다. 폼 오류가 있을 경우 해당 템플릿을 include하여 오류 메시지를 화면에 표시합니다.
4. 포스트 상세 페이지(`post_detail`뷰)에서 댓글과 해시태그 작성 폼에 대한 유효성 검사를 수행했습니다. 댓글 작성 폼과 해시태그 작성 폼의 유효성 검사 후, 오류가 있을 경우 form_error.html 템플릿을 include하여 오류 메시지를 표시합니다.

위의 작업 내용은 로그인 URL을 설정하고, 폼 유효성 검사를 통해 사용자에게 오류 메시지를 표시하도록 구현하는 등 웹 애플리케이션의 기능과 사용자 경험을 개선하기 위한 작업입니다.

### 학습내용정리:
1.`add_error`: `add_error`는 폼 객체에서 발생한 오류 이외에 다른 오류를 추가하기 위해 사용되는 메서드입니다. `form.add_error('field_name', 'error 내용')`과 같이 폼의 특정 필드에 오류 내용을 추가할 수 있습니다.

`LoginRequiredMixin`: `LoginRequiredMixin`은 사용자가 로그인해야만 접근할 수 있는 클래스를 생성하기 위한 mixin입니다. login_url은 로그인되지 않은 사용자가 해당 클래스에 접근할 때 리디렉션할 경로를 지정하는데, 일반적으로 로그인 페이지로 설정됩니다. 다른 경로로 리디렉션하려면 GET 요청의 렌더링 부분에서 `<input type="hidden" name="next" value="{{ request.get_full_path }}">`와 같이 숨겨진 입력 필드를 사용하여 다음 요청에서 해당 값을 활용하여 리디렉션할 경로를 지정할 수 있습니다.

Django Template Engine의 변수 필터 활용: 템플릿에서 변수 필터를 사용하려면 `{{ value|filter:parameter }}`와 같은 형식을 사용합니다. 단일 필터를 적용할 때는 `{{ value|filter }}`와 같이 사용하며, 여러 필터를 연결하여 사용하려면 `{{ value|filter1|filter2 }}`와 같이 사용할 수 있습니다. 또한, 템플릿 태그와 함께 사용하려면 `{{ template_tag|filter:parameter }}`와 같이 사용할 수 있습니다. 필터에 매개변수를 전달하려면 {{ value|filter:"parameter" }}와 같이 사용해야 합니다. 일반적으로 사용되는 내장 필터에는 `upper`, `lower`, `length`, `default`, `join`, `slice` 등이 있습니다.

이러한 내용을 통해 폼 처리와 인증 관련 기능을 보완하고, Django Template Engine에서 변수 필터를 유연하게 활용할 수 있게 되었습니다.
